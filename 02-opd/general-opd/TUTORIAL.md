...
**本章是蒸馏分支的概念入口**（先于医学配方）。默认后端：verl。

**先回答两个问题，再谈 reverse KL：**

1. **谁生成了训练动作？** —— 当前 **Student** 自己采样（on-policy）。
2. **谁给反馈、反馈什么？** —— **Teacher** 对**同一历史、同一 token** 给 logprob；不是重写答案。

| 量 | 角色 | 本批是否更新 |
| --- | --- | --- |
| old_logp | 采样时学生 | 固定快照 |
| current_logp | 正在训练的学生 | **可训练** |
| teacher_logp | 冻结教师复评 | 固定、停止梯度 |
| 精确全词表 KL | 分析目标 | 本地**不**存全词表；只用采样 token 估计 |
