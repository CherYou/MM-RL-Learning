...
每篇末尾有可在 CPU 上完成的练习。LLM 的 `--smoke` 使用随机微型模型与显式 debug 奖励；具身的 `--smoke` 使用真实物理环境、实际环境奖励和小 MLP。两者都以检查执行为目的，稳定能力结论还需要充分训练、独立评估与多个训练种子。当前已实测的 GPU 案例是 [单卡 GRPO](GRPO_GPU_VALIDATION.md)，不能把它推广成其它章节已经完成 GPU 效果验证。
