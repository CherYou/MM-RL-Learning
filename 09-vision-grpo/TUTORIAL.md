...
**本章默认教学后端：verl + Transformers VLM（`verl.yaml`）。** 不是新的 loss 名称；变化在**条件输入身份**。

**相对 GRPO，改变了什么？**

| 组件 | 文字 GRPO | Vision-GRPO（本章） |
| --- | --- | --- |
| 条件 | 仅问题文本 x | (x, 图像 I + processor extras) |
| 组内一致性 | 同题 | 同题 + **同一张图** |
| old/current/ref | 文字 token 一致即可 | 必须看到**同一视觉输入** |
| 梯度路径 | 文字 logits | 可经视觉特征到可训练视觉模块 |
| mask vs 冻结 | 较少混淆 | 图像位 mask=0 ≠ 视觉参数冻结 |
...
图中四个回答来自同一策略的多次采样，不表示训练四套不同权重。这里的图像问答训练也不等于 VLA 机器人控制。

## 两层检查：先证明“图进来了”，再谈“学会看图”

| 层级 | 检查 | 能说明 | 不能说明 |
| --- | --- | --- | --- |
| L1 输入链路 | 固定文字与候选答案，只替换图片，看 logprob 是否变化 | 图像影响了打分 | 模型已理解图意 |
| L2 能力 | 独立图文题上的正确率（训练前后、与训练隔离） | 条件策略变好 | 论文级 VLM 复现 |

生成时有图、更新时丢图 → ratio 无意义。先核对 extras 随 `Sample` 传递，再评估任务正确率。
