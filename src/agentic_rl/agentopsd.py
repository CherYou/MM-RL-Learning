...
def group_sequence_advantages(
    ...
    *,
    ...
    advantages = [(reward - mean_reward) / (std_reward + epsilon) for reward in rewards]
...
def reshape_turn_advantages(
    ...
    sign = 1.0 if sequence_advantage > 0.0 else (-1.0 if sequence_advantage < 0.0 else 0.0)
    outcome_credits = [
        sign * (belief_after - belief_before_value) for _, _, belief_before_value, belief_after in raw
    ]
    credit_mean = sum(outcome_credits) / len(outcome_credits)
    credit_std = float(np.std(outcome_credits, ddof=0))
    normalized = [(credit - credit_mean) / (credit_std + config.epsilon) for credit in outcome_credits]
    ...
    for index, (
        ...
        multiplier = (1.0 - config.reshape_lambda) + config.reshape_lambda * weight
