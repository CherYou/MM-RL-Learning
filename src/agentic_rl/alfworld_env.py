# Ported from KMnO4-zx/agentic-rl-lab @ d745e6e (Apache-2.0).
# Modified: package-relative imports; shared local data root. No remote training.
...
from .environments import canonical_action
...
def _close_async_workers(environment: Any) -> bool:
    ...
    if not children or not all(hasattr(child, "_pipe") and hasattr(child, "_process") for child in children):
        return False
...
class ALFWorldGroup:
    ...
    def reset(self) -> list[EnvironmentState]:
        ...
        self._admissible_actions = [
            tuple(canonical_action(str(action)) for action in actions) for actions in commands
        ]
        ...
        if resolved != {expected}:
            raise RuntimeError(f"同组环境加载了错误游戏: expected={expected}, actual={sorted(resolved)}")
    ...
    def step(self, actions: list[str]) -> list[EnvironmentStep]:
        ...
        admissible = [
            action in current for action, current in zip(normalized, self._admissible_actions, strict=True)
        ]
        if self._uses_in_process_textworld:
            with _TEXTWORLD_SYNC_LOCK:
        ...
        commands = _column(infos, "admissible_commands", self.group_size, [])
        self._admissible_actions = [
            tuple(canonical_action(str(action)) for action in values) for values in commands
        ]
