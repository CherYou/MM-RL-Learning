...
假设两次表现都得到 0.8 分，但一类题通常只能得到 0.2，另一类题通常能得到 0.95。相同分数代表的改进程度并不相同。

因此，我们常把奖励或回报减去一个参照值 $`b`$：

```math
A=R-b.
```

这个差值叫优势。$`A\gt0`$ 表示比参照更好，$`A\lt0`$ 表示比参照更差。它不是“答案正确”的另一种写法。正确答案也可能和同组其他答案一样好，从而没有相对优势。
...
这解释了为什么可以在不改变该期望梯度的前提下加入合适的参照。实际优势估计还有采样误差；不能把这段证明直接套到所有组内标准化方法。尤其组均值包含当前样本时，需要另外分析其统计性质。

PPO 通常借助价值估计构造优势；GRPO 使用同一问题多次尝试的组内统计。这里先把 $`A`$ 看成已经算好、在本次更新中固定的反馈：
...
**问题二。** 某条回答得到奖励 0.8，参照为 0.9。优势是什么？这个信号与“奖励为正所以一定鼓励”有什么区别？
...
2. $`A=-0.1`$，表示低于参照。优势判断相对表现，不只是看原始奖励是否大于 0。
