...
先分清：**我们手里有什么**，以及 **我们想算什么**。

| | 内容 |
| --- | --- |
| 手里有的 | 一批已经生成的回答，动作记作 \(a\)；它们来自**旧策略** \(\pi_{\mathrm{old}}\)（采样时的概率已存下） |
| 想估计的 | 若由**当前策略** \(\pi_\theta\) 生成，平均奖励大约是多少 |

难点不在“有没有新概率”，而在于：**求和/平均是按“旧策略采到的那些样本”做的，不是按所有可能动作、用新概率加权做的。**

#### （1）若能枚举全部动作，根本用不着比率

固定历史下只有两个动作。旧策略给 `[0.5, 0.5]`，当前策略给 `[0.8, 0.2]`，奖励为 `[1, 0]`。

如果**已经知道全部两个动作**，也**已经知道新策略对它们的概率**，那么“新策略下的平均奖励”就是直接按新概率加权：

```math
\underbrace{0.8\times1+0.2\times0}_{=\,0.8}
\quad\text{即}\quad
\mathbb E_{a\sim\pi_\theta}[R(a)]=\sum_a\pi_\theta(a)R(a).
```

这一步**不需要**任何比率。你说得对：\(\pi_{\mathrm{old}}\times\dfrac{\pi_\theta}{\pi_{\mathrm{old}}}=\pi_\theta\)，代数上并不会多出什么神秘的“补偿”。

#### （2）真正的问题：平均是拿旧样本做的

训练时我们通常**不会**对每个动作都算一遍新概率再求和，而是手里只有若干条**旧策略采出来**的回答，对这些回答的奖励做平均。

若旧策略以 50%/50% 生成两类动作，奖励 `[1, 0]`，那么对旧样本的奖励**直接平均**，估计的是：

```math
\underbrace{0.5\times1+0.5\times0}_{=\,0.5}
\quad\text{即}\quad
\mathbb E_{a\sim\pi_{\mathrm{old}}}[R(a)].
```

这是**旧策略**下的平均奖励，不是当前策略的 0.8。  
也就是说：**“直接平均旧样本的奖励”默认你在回答旧策略的问题**；换到新策略，同样的样本平均会答错题。

#### （3）比率做的事：让“按旧样本平均”算出“新策略的期望”

我们仍用旧样本，但对第 \(i\) 条回答乘上比率再平均：

```math
\frac1N\sum_{i=1}^{N}
\frac{\pi_\theta(a_i)}{\pi_{\mathrm{old}}(a_i)}\,R(a_i),
\qquad a_i\sim\pi_{\mathrm{old}}.
```

在两个动作的例子里，旧策略下动作 1、动作 2 的“出现权重”各是 0.5，比率是 `[1.6, 0.4]`：

```math
0.5\times1.6\times1+0.5\times0.4\times0
=0.8.
```

写开每项：

| 动作 | 旧权重 \(\pi_{\mathrm{old}}\) | 比率 \(\rho=\pi_\theta/\pi_{\mathrm{old}}\) | 乘积 = 新概率 | 奖励 \(R\) | 贡献 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 1 | 0.5 | 1.6 | 0.8 | 1 | 0.8 |
| 2 | 0.5 | 0.4 | 0.2 | 0 | 0 |

每一行的 `旧权重 × 比率` 确实就是新概率——所以总结果仍是 \(0.8= \mathbb E_{\pi_\theta}[R]\)。  
**比率不是发明了一个新期望，而是把“按 \(\pi_{\mathrm{old}}\) 做的加权/平均”改写成“按 \(\pi_\theta\) 做的加权/平均”。** 一般写成换测度恒等式：
...
左边：对**新策略**求期望。  
右边：对**旧策略**求期望，但被积函数多乘一个比率。  
两边相等，是因为求和变量仍是 \(a\)，只是权重从 \(\pi_{\mathrm{old}}(a)\) 换成了 \(\pi_{\mathrm{old}}(a)\cdot\frac{\pi_\theta(a)}{\pi_{\mathrm{old}}(a)}=\pi_\theta(a)\)。

当 \(N\) 条样本都是从 \(\pi_{\mathrm{old}}\) 采出时，对 \(f=\rho R\) 做样本平均，就同时用到了这两件事：样本来自旧策略，权重却把总账算成新策略的。

因此，**就估计 \(\mathbb E_{\pi_\theta}[R]\) 而言，不必用当前策略再生成一批**；旧样本 + 比率即可。注意区分：用当前模型给**同一批旧 token** 重新算 logprob 叫 re-score（得到 \(\rho\)），不是 re-roll。训练外层仍会在策略改得较多之后**重新 rollout**，因为比率估计在 \(\pi_\theta\) 与 \(\pi_{\mathrm{old}}\) 差得过远时方差变大、覆盖变差；同一批旧数据上的多次小更新，才是比率真正要服务的场景。

#### （4）支持集与边界

当前策略给正概率的动作，旧策略不能完全没有覆盖，否则比率分母为 0。温度、top-p 等会改变实际行为分布，应纳入“old”的定义。

另外，**逐 token 使用一个比率，不等于自动精确修正整条轨迹的分布变化**。PPO 的裁剪目标是一个局部替代目标；出现重要性比率，并不表示后续所有估计都已无偏。
