...
$$L_\pi=-\mathbb E_t\left[\min\left(r_t^\pi A_t,
\operatorname{clip}(r_t^\pi,1-\epsilon,1+\epsilon)A_t\right)\right],
...
$$V^{clip}=V_{old}+\operatorname{clip}(V-V_{old},-\epsilon_V,\epsilon_V),$$

$$L_V=\tfrac12\mathbb E_t\left[\max((V-\widehat G)^2,(V^{clip}-\widehat G)^2)\right],\qquad
L=L_\pi+c_VL_V.$$
...
\mathbf1[t=\text{最后有效 token}]R_{task}.$$
