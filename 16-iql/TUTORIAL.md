...
Offline RL（离线强化学习）指训练只能使用固定数据，不能执行新动作获得新的训练反馈。数据可以来自过去策略、控制程序或人工示范；本仓库来自仿真采集。Online RL（在线强化学习）则能在学习期间继续和环境交互。网络连接状态与这个区分无关。

先用行为克隆 BC（Behavior Cloning）作参照：给定状态与记录动作，直接提高记录动作的概率。如果数据混合了好与坏的操作，等权模仿可能学到折中行为。IQL 想利用 reward 与后续状态区分这些记录动作的价值，再更用力模仿好的部分。

Implicit Q-Learning 中的 implicit（隐式）指改善动作价值时不必显式在未见过的动作上求最大值。它不是跳过价值学习：本章仍训练 Q、V 和策略三类网络。我们会先说明为什么不能随便让 actor 追高 Q，再用 expectile 建立偏向较好数据动作的 V，最后将价值差变成模仿权重。
...
固定数据集为 $`\mathcal D=\{(s,a,r,s',d)\}`$，分别记录当前状态、动作、即时奖励、下一状态和真正终止标记。两个 $`Q_{\phi_j}`$ 预测记录动作的回报，$`V_\psi`$ 提供偏向优质动作的状态价值，$`\pi_\theta`$ 学连续动作分布；phi、psi、theta 是三类网络各自的参数。慢速目标 Q 记为 $`\bar\phi_j`$，它们缓慢跟随当前 Q，用来提供较平稳的回归标签。
...
为什么不直接取平均？平均 V 会把好坏动作同等对待；我们希望后续价值回传更偏向数据里较好的选择。为什么不直接取最大？有限数据与估计误差下，单个异常高 Q 可能过度主导目标。Expectile 提供一个由 eta 控制的平滑偏向：eta=0.5 时回到对称平方误差，大于 0.5 时低估高值的惩罚更重。它依旧受数据频率与 Q 误差影响，不是无风险的“选最好”。

## 3. Q 与策略的两个 loss

有了不必查询新动作的 V，Q 就能用它补全未来，而不必像 TD3 那样在下一状态调用策略提出一个未见动作。Q 改好后，策略仍需单独学习：下面把“动作比 V 好多少”变成正的模仿权重。
...
先把权重都设为 1，就得到普通行为克隆的负 log-likelihood（负对数似然）。再加入指数权重，高优势样本在平均损失中占比更大，低优势样本仍以正权重存在。这个变化与 PPO 用负优势反向鼓励不同。权重停止梯度，是为了让 actor 模仿由价值网络给出的评价，而不是通过改变评价来降低自己的 loss。

## 4. 两个手算例子

Expectile 名字陌生，先把网络退化成一个待学习的标量 v。这样可直接算出最优值，再把同样的原理理解成网络在许多状态上同时回归。
...
把省略的一步写出：对 v 求导，得到 $`0.25v-0.75(2-v)=v-1.5`$；令它为零，所以 v=1.5。这里 0.25 是高估低值动作的权重，0.75 是低估高值动作的权重。不是从数据排序后机械地选第 75% 个元素。
...
纸上已经有 V 回归、Q 回归、加权模仿三个目标，代码阅读时按这三步分别检查可训练参数。尤其注意每次 critic 输入动作来自数据，actor 的学习并不需要先采一个新动作交给 Q 打分。
...
算法是否合理取决于数据里实际记录了什么。因此运行前先读采集方式、奖励与终止语义，再看训练步数。固定数据的 5000 次更新与在线采集 5000 个环境步不能直接比较。
...
最后从低维实验回到更大应用。能在固定数据上重加权模仿，是一种有用的训练思想；它并不能补出数据从未覆盖的观察和任务，也不自动给出视觉或语言表示。
...
这些题分别检查 expectile、正权重、恢复状态与复现边界。机制来源见 [IQL 作者论文](https://arxiv.org/abs/2110.06169)，具体 tanh 策略、更新顺序与数据采集则以本仓库代码为准。
